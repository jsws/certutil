...
// badssl.com certificate chain.
const badsslcomChainPEM = `
-----BEGIN CERTIFICATE-----
...
func TestParsePEM(t *testing.T) {
	...
	certs, err := ParsePEM([]byte(badsslcomChainPEM))
	if err != nil {
	...
	// Check certificate parsed properly
	if certs[0].Subject.CommonName != "*.badssl.com" {
		t.Errorf("Certificate 0 parsed incorrectly %s", err)
	}
	if certs[1].Subject.CommonName != "DigiCert SHA2 Secure Server CA" {
		t.Errorf("Certificate 1 parsed incorrectly %s", err)
	}
}
